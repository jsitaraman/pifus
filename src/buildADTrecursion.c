		       ...
		       int level,int ndim,int nelem, int nav)
{
  ...
  if (nav > 1) {
    ...
    for(i=0;i<nd;i++)
      {
	adtReals[ndim*(*adtCount)+i]=PIFUS_BIGVALUE;
	adtReals[ndim*(*adtCount)+i+nd]=-PIFUS_BIGVALUE;
      }
    //
    for(i=0;i<nav;i++)
      for(j=0;j<nd;j++)
	{
	  ...
	  jjp=jj+nd;
	  //
	  adtReals[ii]=PIFUS_MIN(adtReals[ii],coord[jj]);
	  adtReals[iip]=PIFUS_MAX(adtReals[iip],coord[jjp]);
	}
    //
